  ...
  if(!empty($_GET['project_id'])){
    ...
    if ( !empty($_POST)) {
        ...
        if ($valid) {
            ...
            $sql = "UPDATE project set project_title = ?, sponsor = ?, comments = ?, owner = ?, required_date = ?, project_status =? WHERE project_id  = ?";
            $q = $pdo->prepare($sql);
            $q->execute(array($project_title,$sponsor,$comments,$owner,$required_date,$project_status,$project_id));
            Database::disconnect();
        ...
        $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT project_title,sponsor,comments,owner,required_date,project_status FROM project WHERE project_id  = ?";
        $q = $pdo->prepare($sql);
...
<body>
    <div class="container">
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="updateproject.php?project_id=<?php echo $project_id?>" method="post">
